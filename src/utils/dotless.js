...
function dotLess(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    let pos,
      ...
      newWord = "",
      oneWord = [...str[i]],
      size = oneWord.length - 1;
    for (let j = 0; j <= size; j++) {
      // one word
      newChar = char = oneWord[j]; // default newChar same as current char
      if ((pos = "ةبتثجخذزشضظغفقني".indexOf(char)) !== -1) {
        // we found our dotty char
        newChar = "هٮٮٮححدرسصطعڡٯٮٮ"[pos]; // change it
        if (j === size || (j <= size && !/[ء-غف-ي]/.test(oneWord[j + 1])))
          // is next char non-Arabic letter?
...
console.log(dotLess("بسم الله الرحمن الرحيم اقرأ باسم ربك الذي خلق"));
